...
```{r libraries, warnings = FALSE, echo= FALSE}
library(here)
library(tidyverse)
library(DT)
library(knitr)
...
airq.long = airq %>%
  ...
  select(Date_Time, `PT08.S1(CO)`,`C6H6(GT)`, `PT08.S2(NMHC)`, `T`, AH) %>%
...
```{r Graph of Air Pollutant - daily average}
...
## Research question
In this analysis, we will attempt to determine the (seasonal?) effects of temperature and humidity on the concentration of air pollutants.
