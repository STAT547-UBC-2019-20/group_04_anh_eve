...
We then perform linear regression of all the separate pollutants with temperature and absolute humidity. 


```{r echo=FALSE, warnings = FALSE}
benzene_coefs <- readRDS(here::here("Data", "Benzene.rds"))

yt <- benzene_coefs[2][[1]][2]*airq["Temp"] + benzene_coefs[2][[1]][1]
new_data <- airq
new_data["yt"] <- yt



#ggplot(data=subset(new_data, !is.na(Temp)), aes(x=Temp, y=yt)) + 


plot_bz_w_time <- new_data %>% 
  ggplot() + 
  geom_point(aes(y=Benzene, x=Temp)) +
  geom_line(aes(x=Temp, y=yt, color = 'Regression line'))+
  theme_bw() +
